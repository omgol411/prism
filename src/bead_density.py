...
class BeadDensity():
  ...
  def extend_voxel(self, voxel_center: np.ndarray) -> tuple:
  ...
  def construct_kernel(self, k1: np.ndarray, k2: np.ndarray) -> None:
    if (k1 == k2).all():
      fixed_bead_kc = self.grid.index_to_coordinate(
        self.grid.get_closest_voxel(k1)
      )
  ...
  def return_density(
    ...
    kernel_centers = _get_voxel_centers(
        self.grid.index_to_coordinate(self.k1_ind),
        self.grid.index_to_coordinate(self.k2_ind),
        self.voxel_size
    )
    ...
    return _add_unique_density(
        [self.grid.coordinate_to_oneDindex(pc) for pc in pos_kernel_centers], density, self.num_models
    )

  def return_density_opt(
    self,
    points: np.ndarray,
    radius: float,
    mass: float,
    ...
    return _add_unique_density(
        [
            self.grid.coordinate_to_oneDindex(pc)
            for pc in kernel_centers[voxel_inds]
        ],
        density,
        self.num_models
    )
